...
* **Robot Demonstration Priors**: Pre-trained weights from large-scale robot manipulation datasets establish a highly transferable initialization for dual-arm/humanoid tabletop manipulation inside the pre-trained CLAP-RF weights. (See Section 8 for specific dataset details).
...
* **Colab Footprint**: Fine-tuning the adapters and the CLAP-RF head on embodiment-specific teleoperated datasets takes **4 to 8 hours on a single L4 GPU** (~5-10 Colab credits), preserving your remaining budget for RL adaptations. (See Section 8 for specific dataset details).
...
| **Stage 2: SFT / Mid-training** *(Action Grounding)*<br><br>**Active Components**:<br>• Hierarchical Block DiT Flow Matcher<br>• Trainable MLP Adapters<br>• Tactile/Proprioceptive stream | **Inputs**: Latent states ($s_t$), target state ($s_{\text{target}}$), joint angles/torques, tactile arrays.<br>**Outputs**: Generated multi-step joint trajectory ($a_{t:t+H}$). | • **Conditional Flow Matching (CFM) Loss**: Regresses predicted velocity fields $v_\theta(x_t, t, s_t, s_{\text{target}})$ to match target trajectories.<br>• **CASA Loss**: Contrastive InfoNCE alignment between states and actions. | • Binds the abstract world model dynamics to the concrete physical embodiment.<br>• Trains the policy to imitate expert demonstrations of coordinated manipulation. | • **CFM Val Loss** converges below target threshold.<br>• **JEPA Diagnostics** (drift and no-op ratio) remain stable.<br>• **Basic Reaching Success** $\ge 80\%$ (open-loop in sim). |
| **Stage 3: RL & Exploration** *(Behavior Tuning)*<br><br>**Active Components**:<br>• RAM (Reinforce Adjoint Matching)<br>• Discriminator ($D_\psi$)<br>• d-OPSD / PF-OPSD<br>• EBT-Policy (MCMC / Langevin)<br>• RATs Task Proposer<br>• `pycapacity` Task-Space filter / URDF Safeguards | **Inputs**: On-policy simulator states, dense progress rewards, adversarial perturbations.<br>**Outputs**: Fine-tuned flow fields, programmatic skills in library. | • **RAM Velocity Correction Loss**: Direct regression weighting of flow based on scalar rewards.<br>• **Contrastive Energy Loss**: Minimizes prediction error for successes; maximizes ($E \to \infty$) for failures.<br>• **Safety Constraint Loss**: Projects trajectories into physical limits using `pycapacity` Cartesian 6DoF boundaries. | Aligns policies to perform high-precision grasp, pinch, and lift operations.<br>• Develops robust recovery skills from catastrophic failures through self-distillation.<br>• Enforces robot-specific joint and torque constraints safely. | • **Custom Task Success Rate** $\ge 90\%$ (grasp, lift, place success).<br>• **Energy Delta** (Clear separation: $E_{\text{fail}} \gg E_{\text{success}}$).<br>• **Workspace Polytope Violation Rate** $= 0\%$ (using the `pycapacity` filter).<br>• **d-OPSD Recovery Rate** $\ge 90\%$ under active physical perturbations. |
...
* **Embodiment-Specific Safety Deferral**: The CPU-bound `SafetyFilter` (URDF limits and `pycapacity` Cartesian workspace projections) is **completely disabled during Stage 2 SFT**. SFT trains the model via behavioral cloning on expert demonstrations, which are already physically safe and valid. Applying robot-specific safety limits to a mixed-robot training dataset (containing different joint structures like ALOHA and T-REX) is methodologically incorrect. Thus, physical safety filtering is deferred to **Stage 3 (RL/inference/deployment time)**, where the model runs on a specific robot embodiment and executes physical rollouts. This simplifies SFT loss calculations and accelerates training by ~15x.
...
* **Task 2.1: Dataset Selection**: Select a diverse, multi-task tabletop manipulation dataset containing human-teleoperated episodes. (See Section 8 for specific dataset details).
