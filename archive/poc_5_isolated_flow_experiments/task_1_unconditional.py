...
sol = solve_ivp(
    flow_ode, t_span=(0.0, 1.0), y0=x0_test, t_eval=eval_times, method="RK45"
)
...
plt.subplot(1, 3, 1)
plt.scatter(
    x1_data[:, 0], x1_data[:, 1], color="gray", alpha=0.2, s=8, label="Target Data"
)
plt.scatter(
    x1_gen[:, 0], x1_gen[:, 1], color="crimson", alpha=0.5, s=8, label="Generated"
)
...
for i, path in enumerate(trajectories):
    plt.plot(
        path[:, 0], path[:, 1], "--o", markersize=3, alpha=0.8, label=f"Path {i+1}"
    )
    plt.scatter(
        path[0, 0], path[0, 1], color="black", marker="s", s=35, zorder=5
    )  # Start (Noise)
    plt.scatter(
        path[-1, 0], path[-1, 1], color="red", marker="*", s=60, zorder=5
    )  # End (Data)
