...
class BadWorldAttacker:
    ...
    def generate_stochastic_ensemble_pass(
        ...
        for var_idx in range(ensemble_size):
            ...
            temp_payload = copy.deepcopy(raw_data)  # current observation
            perturbed_frames = {}
            ...
            with torch.no_grad():
                ...
                perturbed_latent_list.append(s_t_variant)

        # Capture the 4 distinct visual variations generated by the loop
        # [ensemble_size, latent_dim]
        perturbed_latent_list = torch.cat(perturbed_latent_list, dim=0)
        ...
        # Expand target conditioning contexts uniformly to match the full 16-batch
        # profile [16, 512]
        ...
        print(
            "🕵️‍♂️ [ATTACKER DIAGNOSTIC] Replicated Latent Batch Context -> Mean: "
            f"{latent_mean:.4f} | Std: {latent_std:.4f}"
        )
        ...
        print(
            "🕵️‍♂️ [ATTACKER DIAGNOSTIC] Target Latent Context -> Mean: "
            f"{s_target.mean().item():.4f} | Std: {s_target.std().item():.4f}"
        )
        ...
        a_candidates = flow_matcher.sample_with_steering(
            ...
            step_nft_scale=0.45,
        )  # Output Shape: [16, 7, 58]

        # Inspect outputs right out of the flow generator
        with torch.no_grad():
            raw_gen_min = a_candidates.min().item()
            raw_gen_max = a_candidates.max().item()
            print(
                "🕵️‍♂️ [ATTACKER DIAGNOSTIC] Post-Generation Actions -> "
                f"Min: {raw_gen_min:.4f} | Max: {raw_gen_max:.4f}"
            )

        # Return the 4 raw image variants, the 16 ensembled latents, and the 16
